...
#include "./m03_run_length_array.h"
#include "./m03_suffix_index_array.h"
#include <memory>
#include <cstdint>
#include <iostream>
#include <chrono>
...
namespace maniscalco 
{
    ...
    private:
        ...
        public:
            ...
            context_range
            (
                m03_context_array::iterator begin,
                m03_context_array::iterator end
            ):
                read_(begin),
                write_(begin),
                begin_(begin),
            ...
            m03_context_array::iterator get_read() const{return read_;}
            m03_context_array::iterator get_write() const{return write_;}
            m03_context_array::iterator get_end() const{return end_;}
            m03_context_array::iterator get_begin() const{return begin_;}
            ...
            void write
            (
                m03_context_array::iterator where,
                std::uint32_t size,
                m03_context_array::type flag
            )
            {
                if (write_ < where)
            ...
            )
            {
                ...
                if (size < existingSize)
                {
                    ...
                    auto toType = flag;
                    if (fromType == m03_context_array::type::context_end)
                    {
                        toType = m03_context_array::type::child;
                        fromType = m03_context_array::type::context_end;
                    }
                ...
                else
                {
                    auto fromType = write_->get_type();
                    auto toType = flag;
                    if ((fromType == m03_context_array::type::child) && (toType == m03_context_array::type::context_end))
                        toType = m03_context_array::type::child;
                    write_->set(size, toType);
                }
                write_ += size;
            ...
            {
                if (write_ != end_)
                    write_->set(end_ - write_, m03_context_array::type::skip);
                write_ = begin_;
                read_ = begin_;
            }
            ...
            )
            {
                ...
                if (write_ < endSafe)
                {
                    ...
                    skipStart->set(skipSize, m03_context_array::type::skip);
                    if (write_ > endSafe)
                    {
                        endSafe->set((write_ - endSafe), m03_context_array::type::skip);
                        write_ = endSafe;
                    }
                }
                ...
                if ((write_ < where) && (write_->is_skip()))
                {
                    ...
                    while ((write_ < where) && (write_->is_skip()))
                        write_ += write_->get_size();
                    skipStart->set((std::min(write_, where) - skipStart), m03_context_array::type::skip);
                    if (write_ > where)
                    {
                        where->set((write_ - where), m03_context_array::type::skip);
                        write_ = where;
                    }
                }
                while (write_ < where)
        ...
        private:
            ...
            m03_context_array::iterator read_;

            m03_context_array::iterator write_;

            m03_context_array::iterator begin_;

            m03_context_array::iterator end_;
        };
        ...
        (
            m03_run_length_array::const_iterator,
            std::vector<std::uint32_t> const &
        );

        m03_context_array               contextArray_;

        m03_run_length_array            runLengthArray_;

        m03_suffix_index_array          symbolIndexArray_;
    ...
    //==========================================================================
    void m03_encoder::model_next_order_contexts
    (
        m03_run_length_array::const_iterator runLengthArrayIter,
    ...
    )
    {
        ...
        for (auto contextSize : contextLength)
        {
            ...
            while (childContextSymbolListEnd != childContextSymbolList)
            {
                ...
                if (contextStats.childCount_ != contextStats.parentCount_)
                {
                    ...
                    contextStats.contextRange_.write(contextStats.deferred_, m03_context_array::type::child);
        ...
        while (parentContextSymbolListEnd != parentContextSymbolList)
        {
            ...
            if (contextStats.childCount_ != contextStats.parentCount_)
                contextStats.contextRange_.write(contextStats.deferred_, m03_context_array::type::context_end);
    ...
    //==========================================================================
    void m03_encoder::model_contexts
    (
    )
    {
        ...
        for (auto currentContext = contextArray_.begin(); currentContext != contextArray_.end(); )
        {
            ...
            currentContext->set(size, m03_context_array::type::skip);
            currentContext += size;
        }
...
*/
            for (std::size_t i = 0; i < symbolListSize_; ++i)
            {
                ...
                while (contextRange.get_read() != contextRange.get_end())
                {
                    ...
                    while (contextInfo.type_ == m03_context_array::type::skip)
                    {
                    ...
                    if (contextInfo.type_ != m03_context_array::type::skip)
                    {
                        ...
                        bool endParentContext = (currentChildContext->set_type(m03_context_array::type::skip) == m03_context_array::type::context_end);
                        contextLength.clear();
                        ...
                        while (!endParentContext)
                        {
                            ...
                            contextInfo = contextRange.read();
                            endParentContext = (currentChildContext->set_type(m03_context_array::type::skip) == m03_context_array::type::context_end);
                            contextLength.push_back(contextInfo.size_);
                        }
...
) -> std::vector<std::uint8_t>
{
    ...
    m03_encoder encoder(begin, end, sentinelIndex);
